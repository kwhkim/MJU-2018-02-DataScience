...
ggplot(data = data, aes(x=reorder(nation,sum_all), y=sum_all, fill = gender)) + geom_col(position = "dodge") + coord_flip() + labs(title='국가별 남녀 유학생 현황', x='유학생 수', y='국가', caption='2017년도에는 중국 > 베트남 > 몽골 > 우주베키스탄 > 파키스탄 > 
        ...
        중국과 몽골은 여성 유학생이 많으며, 베트남은 남성 유학생이 많다.') + theme_classic()
...
#####- 국가, 성별로 그룹화 한 뒤 여성데이터와 남성 데이터만을 추출하여 D2와 D4라는 새로운 변수를 생성하여 체류자격별 유학생 수를 구하기 
```{r}
library(dplyr)
...
####위에서 구한 데이터에서 D2와 D4자격 각각의 유학생 수가 많은 경우로 한 번 더 나누기 
```{r}
d2_fem <- vi_fem %>% 
  filter(D2>=D4) %>% 
  arrange(desc(D2)) %>% 
...
```{r}
library(tidyr)
d2_f <- gather(d2_fem, visa, val, D2:D4)
d4_f <- gather(d4_fem, visa, val, D2:D4)

d2_m <- gather(d2_male, visa, val, D2:D4)
d4_m <- gather(d4_male, visa, val, D2:D4)
```
...
ggplot(data = d2_f, aes(x=reorder(nation,  val), y=val, fill = visa)) + geom_col(position = "dodge", aes(fill = nation)) + coord_flip() + labs(title = '국가별 D2자격 현황', x='국가', y='학생 수', subtitle='한국유학비자 D-2체류자격을 보유한 여성 유학생', caption='2017년도에는
                       중국 > 베트남 > 미국 > 싱가포르 > 일본 > 말레이시아 순으로
                       한국유학비자 D-2 체류자격을 보유한 여성 유학생이 많다.') + theme_classic()

ggplot(data = d2_m, aes(x=reorder(nation,  val), y=val, fill = visa)) + geom_col(position = "dodge", aes(fill = nation)) + coord_flip() + labs(title = '국가별 D2자격 현황', x='국가', y='학생 수', subtitle='한국유학비자 D-2체류자격을 보유한 남성 유학생', caption='2017년도에는
                       중국 > 베트남 > 말레이시아 > 싱가포르 > 미국 > 스리랑카순으로
                                                                                                         한국유학비자 D-2 체류자격을 보유한 남성 유학생이 많다.') + theme_classic()

ggplot(data = d4_f, aes(x=reorder(nation, val), y=val, fill = visa)) + geom_col(position = "dodge", aes(fill = nation)) + coord_flip()  + labs(title = '국가별 D4자격 현황', x='국가', y='학생 수', subtitle='한국 어학연수비자 D-4체류자격을 보유한 여성 유학생', caption='2017년도에는
                       몽골 > 타이완 > 홍콩 > 우즈베키스탄 > 러시아 >
                       태국 > 인도네시아 > 스리랑카 > 미얀마 > 카자흐스탄
                       순으로 한국 어학연수비자 D-4 체류자격을
                       보유한 여성 유학생이 많다.') + theme_classic()

ggplot(data = d4_m, aes(x=reorder(nation, val), y=val, fill = visa)) + geom_col(position = "dodge", aes(fill = nation)) + coord_flip() + labs(title = '국가별 D4자격 현황', x='국가', y='학생 수', subtitle='한국 어학연수비자 D-4체류자격을 보유한 남성  유학생', caption='2017년도에는
                      우즈베키스탄 > 몽골 > 인도 > 네팔 > 일본 > 미얀마 >
                      파키스탄 > 말리 > 한국계 중국인 > 인도네시아 순으로
                      한국 어학연수비자 D-4 체류자격을 보유한 남성 유학생이 많다.') + theme_classic()
...
![](https://bit.ly/2znRN0c)

---

####한국유학비자(D2)체류자격은 남녀 유학생 모두 중국 > 베트남 순으로 학생 수가 많음

####한국 어학연수비자(D4)체류자격에서 여성 유학생은 몽골 > 타이완 > 홍콩 순으로 많고,  남성유학생은 우즈베키스탄 > 몽골순으로 유학생이 대부분을 차지함
...
####4. 한국유학비자(D2)체류자격은 남녀 유학생 모두 중국 > 베트남 순으로 학생 수가 많으며, 한국 어학연수비자(D4)체류자격은 여성 유학생은 몽골, 남성유학생은 우즈베키스탄과 몽골의 유학생이 많음
